...
chrome.storage.local.get(["fixed"], ({ fixed }) =>
  document.body.classList.toggle(FIXED_CLS, fixed)
);
...
const mutationCallback = ([mutation]) => {
  ...
  document.body.style.setProperty("--fytp-player-height", height);
};
...
const resizeCallback = ([entry]) => {
  ...
  document.body.style.setProperty("--fytp-primary-colomn-width", width + "px");
};
